// main.c, 159
// OS bootstrap and kernel code for OS phase 9
//
...
int main(void) {  // OS bootstraps
   ...
   for(i=0;i<4;i++)
   {
      MyBzero((char *)&mutex[i],sizeof(mutex_t));
      mutex[i].lock = UNLOCK;
   }
   for(i=0 ; i<PAGE_NUM;i++)
   {
	    page[i].addr = 0xe00000 + i*PAGE_SIZE;
	    page[i].lrucount = 0;
	    page[i].ownerpid = -1;
      MyBzero((char *)page[i].addr, PAGE_SIZE);                /// do it later
   ...
   switch(proc_frame_p -> event_type) {   //might just do ifs...
     ...
     case SYSCALL_EVENT :
        if(proc_frame_p -> EAX == WRITE)	WriteHandler(proc_frame_p);
	       ...
	       else if(proc_frame_p -> ECX == UNLOCK)	MutexUnlockHandler(proc_frame_p -> EBX);
        }
	      ...
	      else if(proc_frame_p -> EAX == EXIT)		  ExitHandler(proc_frame_p);
	      else if(proc_frame_p -> EAX == WAITPID)	  WaitPidHandler(proc_frame_p);
	      else if(proc_frame_p -> EAX == EXEC)	  ExecHandler(proc_frame_p);
