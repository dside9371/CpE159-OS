...
void MutexLockHandler(int mutex_id){
	if(mutex[mutex_id].lock == UNLOCK) mutex[mutex_id].lock = LOCK;
	else{
		EnQ(run_pid, &mutex[mutex_id].wait_q);
		pcb[run_pid].state = WAIT;
		run_pid=-1;
	}
}
void MutexUnlockHandler(int mutex_id){
      int pid;
      if(mutex[mutex_id].wait_q.size == 0) mutex[mutex_id].lock = UNLOCK;
      else if(mutex[mutex_id].wait_q.size != 0){
          pid=DeQ(&mutex[mutex_id].wait_q);
          EnQ(pid,&run_q);
          pcb[pid].state = RUN;
      }
}
