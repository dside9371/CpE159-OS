// main.c, 159
// OS bootstrap and kernel code for OS phase 7
//
   ...
   switch(proc_frame_p -> event_type) {   //might just do ifs...
     ...
     case SYSCALL_EVENT :
	  ...
	  if(proc_frame_p -> EBX == UNLOCK)	MutexUnlockHandler();
        ...
        if(proc_frame_p -> EAX == PUTCHAR)     PutCharHandler(proc_frame_p);
        if(proc_frame_p -> EAX == FORK)     ForkHandler(proc_frame_p);           //PHASE7
        break;
     case TERM1_EVENT:
        TermHandler(TERM1);
        outportb(0x20, 0x63);
